...
player_actarray_actuator_t glowa;
...
keyboard_handler(void* arg)
{
  ...
  puts("   m    ,    .");
  puts("");
  puts("10/9 : increase/decrease max speeds by 10%");
  puts("8/7 : increase/decrease only linear speed by 10%");
  puts("6/5 : increase/decrease only angular speed by 10%");
  if(use_gripper)
    {
  ...
  for(;;)
  {
    ...
    case KEYCODE_A:
      aktuator.index=1;
      aktuator.value=SERVO_QUANT;
      aktuator.changed=true;
      break;
    case KEYCODE_D:
      aktuator.index=1;
      aktuator.value=-SERVO_QUANT;
...
Client::Client(char* host, int port )
{
  ...
  if(use_gripper)
    {
      printf("ACTARRAY\n");
      gp=NULL;
      lp = new ActArrayProxy(player,idx);
      assert(lp);
    }
  else
  {
  ...
  lp->RequestGeometry();
  Read();
  printf("%s %d %d %d\n",lp->GetDriverName().c_str(), lp->IsValid(), lp->IsFresh(),lp->GetCount());
}
  ...
  if(cont->dirty || always_command) // if the joystick sent a command
  {
    ...
    lastcommand = curr;
          ...
          (((curr.tv_sec + (curr.tv_usec / 1e6)) -
           ...
           COMMAND_TIMEOUT_SEC)){
    if (aktuator.changed) {
      if (aktuator.index==-1) lp->MoveHome(aktuator.index);
      else if( lp ) {
	//lp->RequestGeometry();
	//printf("Glowa %d\n",lp->GetCount());
	glowa = lp->GetActuatorData(aktuator.index);
	//printf("pozycja: %d %f\n",aktuator.index,glowa.position);
	lp->MoveTo(aktuator.index,glowa.position+aktuator.value);
      }
      aktuator.changed=false;
    }
...
int main(int argc, char** argv) {
  ...
  for( int i=1; i<argc; i++ ) {
    ...
    else if( strcmp( argv[i], "-v" ) == 0 )   g_verbose = true;
    else if( strcmp( argv[i], "-3d" ) == 0 )  threed = true;
    else if( strcmp( argv[i], "-c" ) == 0 )   always_command = true;
    else if( strcmp( argv[i], "-n" ) == 0 )   debug_mode = true;
    else if( strcmp( argv[i], "-p" ) == 0 )   print_speeds = true;
    else if( strcmp( argv[i], "-k" ) == 0 )   use_keyboard = true;
    else if( strcmp( argv[i], "-g" ) == 0 )   use_gripper = true;
  ...
  if( clients.begin() == clients.end() )
    clients.push_front( new Client( (char*)DEFAULT_HOST, DEFAULT_PORT ));
